...
router.post('/addToCart',(req,res,next)=>{
	var result = { code:1 }
	CartModel.find({productName:req.body.productName},(err,docs)=>{
		if(err || docs.length!==0){
			result.message = '商品已存在';
		}else{
			var addToCart = new CartModel();		
...
router.post('/getShopCart',(req,res,next)=>{
	var result = { code:1 }
	CartModel.find({productOwner:req.body.productOwner,flag:1},(err,docs)=>{
		if(err || docs.length = 0 ){
...
router.post('/delProduct',(req,res,next)=>{
	var result = { code:1 };
	CartModel.find({_id:req.body.id},(err,docs)=>{
		if(err || docs.length ===0 ){
